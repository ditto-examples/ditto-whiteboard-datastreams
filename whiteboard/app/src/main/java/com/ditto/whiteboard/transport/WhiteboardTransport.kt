...
interface WhiteboardTransport : AutoCloseable {
  ...
  suspend fun sendReliable(operation: BoardOperation)
  fun sendLive(preview: LivePreview)
  fun resolvePermissions(allGranted: Boolean) = Unit
}

sealed interface TransportEvent {
  data class ReliableOperationReceived(val operation: BoardOperation) : TransportEvent
  data class LivePreviewReceived(val preview: LivePreview) : TransportEvent
  data class SnapshotMerged(val operations: List<BoardOperation>) : TransportEvent
...
data class PeerDiagnostics(
  ...
  val snapshotStatus: String = "Idle",
